...
local function run(msg,matches) 
    local text = '*Thanks For Start Funny Bot*' 
    local channel_id = get_receiver_api(msg) 
    local link_text = 'Bot Pm Resan TM' 
    local data = load_data(_config.moderation.data) 
    local link = 'https://telegram.me/spherochbot'
    local keyboard = '{"inline_keyboard":[[{"text":"'..link_text..'","url":"'..link..'"}],[{"text":"Join to Sphero Channel","url":"https://telegram.me/Spheroch"}],[{"text":"BlackLife Channel","url":"http://telegram.me/blacklifech"},{"text":"Dev Bot","url":"http://telegram.me/mrblacklife"}]]}' --به دلخواه متن ها را تغییر دهید 
